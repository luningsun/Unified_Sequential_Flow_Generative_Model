...
class DeepARNetwork(nn.Module):
    ...
    def get_lagged_subsequences(
        ...
        for lag_index in indices:
            print('lag_index is', lag_index)
            begin_index = -lag_index - subsequences_length
            end_index = -lag_index if lag_index > 0 else None
            print('begin_index is', begin_index)
            print('end_index is', end_index)
            lagged_values.append(sequence[:, begin_index:end_index, ...])
        print('\n')
        print('calculate the lagged sequences')
        #pdb.set_trace()
    ...
    def unroll_encoder(
        ...
        if future_time_feat is None or future_target is None:
            ...
            ##
            print('prediction_length is self-defined, what is history length and context length?')
            # history length = context_length+max(lags_seq)
            print('\n')
            #pdb.set_trace()
        ...
        ## why do we need a get_lagged_subsequences here
        ...
        print('\n')
        print('before rnn')
        print('input_lags is',input_lags.shape)
        print('time_feat is',time_feat.shape)
        print('reapeated_static_feat is', repeated_static_feat.shape)
        #pdb.set_trace()
        ## added by lsun, hack to only use time series as input, if requires_grad is needed?
        #inputs = past_target.unsqueeze(2)
        inputs = input_lags
...
class DeepARTrainingNetwork(DeepARNetwork):
    def distribution(
        ...
        ## 
        print('\n')
        print('before calling the unroll_encoder')
        #pdb.set_trace()
        ...
        print('after calling the unroll_encoder')
        #pdb.set_trace()
    ...
    def forward(
        ...
        print('why the taret has only 2 dims')
        #pdb.set_trace()
        target = torch.cat(
            (
...
class DeepARPredictionNetwork(DeepARNetwork):
    ...
    def sampling_decoder(
        ...
        ### shape of examples
        ### past_target: [1,24], repeatexd_past_target: [100, 24]
        ### time_feat: [1, 12, 6], repeated_time_feat: [100, 12, 6]
        ### static_feat: [1, 3], repeated_static_feat: [100, 1, 3]
        ### scale: [1, 1], repeated_static_feat: [100, 1]
        ...
        for k in range(self.prediction_length):
            ...
            ## hack by lsun, remove some time featrues and make train/sample consistent
            decoder_input = input_lags
