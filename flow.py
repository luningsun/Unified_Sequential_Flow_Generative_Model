...
class Flow(nn.Module):
    def __init__(self, input_size, adj_var=1):
        super().__init__()
        self.__scale = None
        self.net = None
        print('using flow with var ', str(adj_var))
        # base distribution for calculation of log prob under the model
        self.register_buffer("base_dist_mean", torch.zeros(input_size))
        self.register_buffer("base_dist_var", adj_var*torch.ones(input_size))
    ...
    def log_prob(self, x, cond):
        #print('compute log_prob')
        #pdb.set_trace()
        u, sum_log_abs_det_jacobians = self.forward(x, cond)
        
        return torch.sum(self.base_dist.log_prob(u) + sum_log_abs_det_jacobians, dim=-1)

    def sample(self, sample_shape=torch.Size(), cond=None, sample_flag = True):
        if cond is not None:
            shape = cond.shape[:-1]
        else:
            shape = sample_shape
        ...
        if sample_flag == False:
            ## switich between samples or mean
            #print('sample from mean')
...
class RealNVP(Flow):
    def __init__(
        ...
        batch_norm=True,
    adj_var = 1):
        super().__init__(input_size, adj_var = adj_var)
